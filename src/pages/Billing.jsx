...
const GenerateBillModal = ({ onClose, preselectedCustomer }) => {
    ...
    const initForm = (c) => {
        const base = { generatedDate: new Date().toISOString().split('T')[0], tvAmount: 0, internetAmount: 0, totalAmount: 0, serviceType: '', generatedBy: user?.username || user?.userId || '' };
        if (!c) return base;
                            ...
                            <div className="bm-form-grid">
                                ...
                                <div className="form-group">
                                    ...
                                    <select
                                        ...
                                        onChange={e => setFormData(p => ({ ...p, generatedBy: e.target.value }))}
                                        disabled={user?.role?.toLowerCase() !== 'owner'}
                                    >
                                        {!users.find(u => u.id === formData.generatedBy) && user && (
